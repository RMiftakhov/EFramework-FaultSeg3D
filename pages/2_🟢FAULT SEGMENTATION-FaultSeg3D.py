...
with st.expander("🟢 Step 1 - To work with this APP we need to load seismic into RAM"):
    ...
    if st.session_state['FaultSeg3D']['numpy_data'] is not None:    
        step1_crop_viz = VISUALIZATION(st.session_state['FaultSeg3D']['numpy_data'] , st.session_state.seismic_type)
        step1_crop_viz.viz_data_3d(st.session_state['FaultSeg3D']['numpy_data'] , key = 20, is_fspect=False)
...
if st.session_state['FaultSeg3D']['is_predicted']:
    step2_viz = VISUALIZATION(st.session_state['FaultSeg3D']['numpy_result']  , st.session_state.seismic_type)
    step2_viz.viz_sidebyside_3d(st.session_state['FaultSeg3D']['numpy_data'], st.session_state['FaultSeg3D']['numpy_result'] , key=30)
